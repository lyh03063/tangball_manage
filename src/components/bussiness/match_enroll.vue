    ...
    <div class="panel">
      ...
      >
        ...
        <template v-slot:slot_detail_item_memberId="{row}">
          <dm_ajax_populate :id="row.memberId" populateKey="name" page="tangball_member" idKey="P1"></dm_ajax_populate>
        </template>
        <template v-slot:slot_detail_item_matchId="{row}">
          <dm_ajax_populate
            :id="row.memberId"
            populateKey="matchName"
            page="tangball_match"
            idKey="P1"
          ></dm_ajax_populate>
        </template>
          ...
          <div class v-if="row.teamDoc">
            ...
            <el-popover placement="right" width="300" trigger="click" >
              ...
              <el-link type="primary" slot="reference" @click="showProgress(row.teamDoc.member)">
                ...
                <font
                  color="red"
                  v-if="isEntering(row.teamDoc.member)!=0&&playerPhoneList.length>0"
                >有{{isEntering(row.teamDoc.member)}}球员未入库</font>
              </el-link>
            </el-popover>
          </div>
        </template>
    ...
    handleCFlist: async function() {
      if (this.matchInfo.matchForm == 1) {
        ...
        this.cfList.formItems =
          PUB.listCF.tangball_match_enroll_single.formItems;
        this.cfList.columns = PUB.listCF.tangball_match_enroll_single.columns;
        this.cfList.detailItems =
          PUB.listCF.tangball_match_enroll_single.detailItems;
      }
      if (this.matchInfo.matchForm == 2) {
        this.title = "报名球队信息";
        //动态数据字典，获取队伍信息
        this.cfList.detailItems =
          PUB.listCF.tangball_match_enroll_team.detailItems;
        ...
        (this.cfList.cfForm = {
          ...
          (this.cfList.formItems =
            PUB.listCF.tangball_match_enroll_team.formItems);
      }
    },
    async getMatchData() {
      if (!this.matchId) return;
