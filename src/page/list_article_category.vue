<template>
  <div class>
    <el-button plain @click="filterActicle" size="mini">筛选文章</el-button>

    <listData :cf="cfList">
      <template v-slot:slot_form_item_detailItems="{row}">
        <div v-for="text in row.article" :key="text.title">
          <router-link to="/list_article">{{text.title}}</router-link>
        </div>
      </template>
    </listData>
  </div>
  ...
  list: [
    {
      ...
      articleNumber: "3",
      article: [
        { title: "文章分类唐球规则下的文章一", text: "这是一篇文章" },
    ...
    },
    {
      P1: 2,
      name: "比赛说明",
      remark: "关于唐球比赛的各项规则说明",
      articleNumber: "3",
      article: [
        { title: "文章分类比赛说明下的文章一", text: "这是一篇文章" },
    ...
    },
    {
      ...
      articleNumber: "3",
      article: [
        { title: "文章分类推广赞助下的文章一", text: "这是一篇文章" },
